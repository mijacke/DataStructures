...
namespace ds::mm {
    ...
    BlockType* CompactMemoryManager<BlockType>::allocateMemoryAt(size_t index)
    {
        ...
        if (end_ - base_ > static_cast<ptrdiff_t>(index)) 
		{
			std::memmove(
                base_ + index + 1,
                base_ + index,
    ...
    void CompactMemoryManager<BlockType>::releaseMemory(BlockType* pointer)
    {
        ...
        {
            // finalizuj p
            p->~BlockType(); // volanie destruktora
            ++p;
		}
        end_ = pointer;
    ...
    void CompactMemoryManager<BlockType>::changeCapacity(size_t newCapacity)
    {
        ...
        base_ = reinterpret_cast<BlockType*>(newBase); // mozeme naucit napriklad psa mnaukat, netreba to ale zneuzivat
    ...
    {
        return &data < end_ && &data >= base_ ? &data - base_ : ds::INVALID_INDEX;
    }

    template<typename BlockType>
    BlockType& CompactMemoryManager<BlockType>::getBlockAt(size_t index)
    {
        return *(base_ + index); // *(base_ + index) je to iste ako base_[index], len to je o nieco rychlejsie
    }
