...
namespace ds::utils
{
    ...
    ListAnalyzer<List>::ListAnalyzer(const std::string& name) :
        ...
        data_(0)
    {
        // Registracia operacie pred vykonanim, aby sa zabezpecilo, ze sa vygeneruje nahodne cislo
    ...
    {
        // Odstranenie zaciatocneho prvku, ak je zoznam neprazdny
        structure.erase(structure.begin());

        // Odstranenie zaciatocneho prvku, ak je zoznam neprazdny, moje riesenie
        //if (!structure.empty()) {
        //    auto it = structure.begin();
        //    std::advance(it, getRandomIndex() % structure.size()); // Uistenie sa, ze index je v rozsahu zoznamu
        //    structure.erase(it);
        //}
    }
